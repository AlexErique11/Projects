...
DATA_PATH = os.path.join(SCRIPT_DIR, "data", "lichess_data.zst")  # your .zst file
MODEL_PATH = os.path.join(SCRIPT_DIR, "human_playability_model.json")  # XGBoost JSON model
FEATURES_CSV = os.path.join(SCRIPT_DIR, "features.csv")
LAST_GAME_FILE = os.path.join(SCRIPT_DIR, "last_game.json")
...
MAX_GAMES = 1000  # train on 1000 games for testing
game_count = 0
    ...
    with open(DATA_PATH, 'rb') as f:
        ...
        try:
            while game is not None and game_count < MAX_GAMES:
                ...
                board = game.board()
                game_positions = []  # temporary storage for this game
            ...
            print("\nProgram stopped manually. Saving features...")
            if positions:
            ...
            print(f"Features saved. Total games processed: {df_features['game_number'].max()}")
            exit()
...
df_features["label_move_ease"] = df_features.apply(
    lambda row: 1 if row["human_move"] in eval(row["top_moves"]) else 0,
    axis=1
)
