...
for elo_range in elo_ranges:
    ...
    for tc in time_controls:
        ...
        metrics = model_metrics.get(elo_range, {}).get(tc, {}).get("label_move_ease", {})
...
print("\n--- Move Ease Stats (label_move_ease) by Elo and Time Control ---")
...
for tc in time_controls:
    model_file = os.path.join(MODEL_DIR, f"model_{elo_range}_{tc}_label_move_ease.pkl")
    if not os.path.exists(model_file):
        print(f"Model file for Elo '{elo_range}', Time '{tc}' not found: {model_file}")
        continue
    ...
    print(f"\n--- Feature Importance for Elo {elo_input} (Range {elo_range}, Time {tc}) - label_move_ease ---")
    ...
    try:
        ...
        plt.title(f"Feature Importance for Elo {elo_input} (Range {elo_range}, Time {tc}) - label_move_ease")
        plt.show()
    except ImportError:
